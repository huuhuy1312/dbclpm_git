...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        ...
        if (user != null) {
            ...
            {
                return ResponseEntity.ok(new CustomerLoginResponse(user.getRole(),user.getCustomer().getOrdinal_numbers(),user.getCustomer().getInfoCommon().getName(),user.getCustomer().getId()));
            }
            else {
    ...
    public ResponseEntity<?> acceptRequest(@RequestBody CreateUserRequest createUserRequest)
    {
    ...
    {
        try {
            if(Objects.equals(rejectRequest.reason, ""))
            {
                return ResponseEntity.badRequest().body("Chưa nhập lý do từ chối");
            }
            customerRepository.findById(rejectRequest.customer_id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy invoice có id=" + rejectRequest.customer_id));
    ...
    ///Chưa test
    public ResponseEntity<?> sendMail(String emailTo, String content, String subject)
    {
        try{
