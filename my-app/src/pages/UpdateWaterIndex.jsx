...
function UpdateWaterIndex() {
  ...
  const handleChangeMonth = (value, index) => {
    setNewMonth(prevValues => {
      const updatedMonths = [...prevValues];
    ...
    const convertDateFormat = (inputDate) => {
      const yearMonth = inputDate.split('-').reverse().join('/');
      return yearMonth;
  }
  const handleUpdate = async (id,part,index) => {
    if (newWaterValues1[`${part}_${id}`]?.value === '') {
      ...
      if(part ==1)
      {
        ...
        const response = await axios.post(url,{
          ...
          period: convertDateFormat(newMonth[index])
        });
        console.log(response.data)
        showNotification()
        setNewWaterValues1(prevValues => ({
                  ...
                  {listResult1?.map((item,index)=>{
                    return(
                      <tr>
                        ...
                        <td style={{textAlign:"center"}}>{item?.waterIndex}</td>
                        <td> 
                          <input
                            type='number'
                        ...
                        <td><button onClick={()=>handleUpdate(item?.id,1,index)}>Cập nhật</button></td>
                      </tr>
                    )
                  })}
                </tbody>
                  ...
                  {listResult2.map((item,index)=>{
                    return(
                      <tr>
                        ...
                        <td><button onClick={()=>handleUpdate(item?.id,2,index)}>Cập nhật</button></td>
                      </tr>
                    )
                  })}
                </tbody>
